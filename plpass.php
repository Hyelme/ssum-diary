...
$result=mysql_query("select writer from place where id=$id",$con);
...
if ($writer != $UserName && $UserID != "admin") {            // 암호가 일치하지 않는 경우(관리자로 로그인 시 삭제,수정 가능)
    ...
    switch ($mode) {
        case 0:          // 수정 프로그램 호출
            echo("<meta http-equiv='Refresh' content='0; url=plmodify.php?board=place&id=$id'>");
            break;
        case 1:          // 삭제 프로그램 호출
            echo("<meta http-equiv='Refresh' content='0; url=pldelete.php?board=place&id=$id'>");
            break;
    }   	   
}  
