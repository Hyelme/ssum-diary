...
");
				...
				if (!$total){
					echo("
						<tr><td class='none_contents' colspan=5>아직 등록된 글이 없습니다.</td></tr>
							...
							<div class='btn_area right'>
								 <a id='writing' class='btns board gray' href='fbinput.php?board=testboard'>
								 글쓰기</a>
							</div>
					");
					...
					echo("
						<tr>
							<th class='no' scope='col'>$id</th>
							<th class='title' scope='col'>$t<a href=fbcontent.php?board=testboard&id=$id>$topic</a></th>
							<th class='writer' scope='col'>$writer</th>
							<th align=center scope='col'>$wdate</th>
							<th align=center scope='col'>$hit</th>
						</tr>
					");
					...
					echo ("<a href=fbshow.php?board=testboard&cblock=$pblock&cpage=$pstartpage><strong>〈</strong></a> ");
					...
					while($i < $nstartpage):
						...
						echo ("<a href=fbshow.php?board=testboard&cblock=$cblock&cpage=$i><strong onclick>$i </strong></a>");
						$i = $i + 1;
					endwhile;
	 
					// 다음 블록의 시작 페이지가 전체 페이지 수보다 작으면 [다음블록] 버튼 활성화  
					 if ($nstartpage <= $totalpage)   
						echo ("<a href=fbshow.php?board=testboard&cblock=$nblock&cpage=$nstartpage><strong>〉</strong></a>");
