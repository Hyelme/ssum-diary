...
");
				...
				if (!$total){
					echo("
						<tr><td class='none_contents' colspan=5>아직 등록된 글이 없습니다.</td></tr>
							...
							<div class='btn_area right'>
								 <a id='writing' class='btns board gray' href='input.php?board=$board' onClick = 'idCheck(<%= $UserID %>'>
								 글쓰기
								 </a>
							</div>
					");
					...
					echo("
						...
						<div class='btn_area right'>
								 <a id='writing' class='btns board gray' href='input.php?board=$board'>
								 글쓰기
								 </a>
							</div>
					");
					...
					echo ("<a href=show.php?board=$board&cblock=$pblock&cpage=$pstartpage><strong>〈</strong></a> ");
					...
					while($i < $nstartpage):
						...
						echo ("<a href=show.php?board=$board&cblock=$cblock&cpage=$i><strong>$i</strong></a>");
						$i = $i + 1;
					endwhile;
	 
					// 다음 블록의 시작 페이지가 전체 페이지 수보다 작으면 [다음블록] 버튼 활성화  
					 if ($nstartpage <= $totalpage)   
						echo ("<a href=show.php?board=$board&cblock=$nblock&cpage=$nstartpage><strong>〉</strong></a>");
