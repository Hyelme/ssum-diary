...
$temp = mysql_query("select * from souvenir where id=$id");

$filename = mysql_result($temp, 0, "filename");



$result = mysql_query("delete from souvenir where id=$id",$con);
...
$tmp = mysql_query("select id from souvenir order by id desc", $con);
$last = mysql_result($tmp, 0, "id");     // 가장 마지막 글 번호 추출

$i = $id + 1;       // 삭제된 글의 번호보다 1이 큰 글 번호부터 변경
while($i <= $last):
	mysql_query("update souvenir set id=id-1 where id=$i", $con);
	$i++;
endwhile;
