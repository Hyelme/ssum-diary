...
$result = mysql_query("select * from souvenir order by id desc", $con);
...
<body>
 ...
 <div class='wrap_tbl tb_info'>
		<table>
			<caption>기념품 공유 게시판 : No, 제목, 작성자, 작성일, 조회수
			</caption>
			<colgroup>
				<col style = 'width : 8%;' />
				<col style = 'width : 55%;' />
				...
				if (!$total){
					echo("
						<tr><td class='none_contents' colspan=5>아직 등록된 글이 없습니다.</td></tr>
							...
							<div class='btn_area right'>
								 <a id='writing' class='btns board gray' href='svinput.php?board=souvenir'>
								 글쓰기</a>
							</div>
					");
					...
					echo("
						<tr>
							...
							<th class='title' scope='col'>$t<a href=svcontent.php?board=souvenir&id=$id>$topic</a></td>
					...
					echo ("<a href=svshow.php?board=souvenir&cblock=$pblock&cpage=$pstartpage><strong>〈</strong></a> ");
					...
					while($i < $nstartpage):
						...
						echo ("<a href=svshow.php?board=souvenir&cblock=$cblock&cpage=$i><strong>$i</strong></a>");
						$i = $i + 1;
					endwhile;
	 
					// 다음 블록의 시작 페이지가 전체 페이지 수보다 작으면 [다음블록] 버튼 활성화  
					 if ($nstartpage <= $totalpage)   
						echo ("<a href=svshow.php?board=souvenir&cblock=$nblock&cpage=$nstartpage><strong>〉</strong></a>");
