...
$result = mysql_query("insert into member(uid, upass,uname, mphone, email, zipcode, addr1, addr2, approved) values ('$uid', '$upass1', '$uname', '$mphone', '$email', '$zipcode', '$addr1', '$addr2', 0)", $con);
	
if ($result) {
    echo ("<script>
		window.alert('[여행을 담다,쑴] 회원 가입을 축하드립니다.\\n관리자의 승인을 거쳐야 로그인이 가능합니다');
		history.go(1);
		</script>
   ");
} else {
