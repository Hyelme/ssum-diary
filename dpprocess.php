...
$result2 = mysql_query("insert into diary(id, dname, dday, sdate, content, userfile, route) values($id, '$dname', $dday, '$sdate', '$content', '$userfile_name', '$route')",$con);
...
if(!$result2) {
	echo("
		  <script>
		  window.alert('사진을 추가하지 못했습니다. 다시 시도해주세요.');
		  history.go(-1)
		  </script>
	");
	exit;
