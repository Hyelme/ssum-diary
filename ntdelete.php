...
mysql_query("delete from notice where id=$id",$con);
echo("
	<script>
...
$tmp = mysql_query("select id from notice order by id desc", $con);
$last = mysql_result($tmp, 0, "id");     // 가장 마지막 글 번호 추출

$i = $id + 1;       // 삭제된 글의 번호보다 1이 큰 글 번호부터 변경
while($i <= $last):
	mysql_query("update notice set id=id-1 where id=$i", $con);
	$i++;
endwhile;
